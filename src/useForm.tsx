...
export default function useForm<TForm extends FormDataType>(
     ...
     (method: Method, url: string, options: VisitOptions = {}) => {
      const _options: VisitOptions = {
        ...
        onBefore: (visit) => {
          ...
          if (recentlySuccessfulTimeoutId.current != null) {
        ...
        onError: (errors) => {
          if (isMounted.current) {
            setProcessing(false)
            setProgress(null)
            setErrors((errors as unknown) as Record<keyof TForm, string[]>)
            setHasErrors(true)
          }
  ...
  return {
    data,
    // @ts-expect-error setData has a different sig, maybe look into this later?
    setData(keyOrData: keyof TForm | ((value: Kaioken.StateSetter<TForm>) => void) | TForm, maybeValue?: TForm[keyof TForm]) {
